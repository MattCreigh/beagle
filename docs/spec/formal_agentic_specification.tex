...
% box-drawing glyphs; compile with XeLaTeX or LuaLaTeX (pdflatex will
% drop or error on those codepoints).
\usepackage{iftex}
...
The lowering function is a four-stage pipeline. Stage 1 merges
...
\begin{itemize}
    ...
    \item Defaults: $\theta_{\text{freq}} \ge 3$, $\theta_{\text{sim}}$ fixed per
        embedding model card. All four operators are deterministic given the
        telemetry snapshot; \texttt{observe} pins the snapshot id so gap
        reports are replayable.
\end{itemize}
...
\begin{enumerate}
    ...
    \item \textbf{Source Trust Tiering}: Every citation must resolve to a source in a trusted origin class (first-party repository files, first-party documentation). Web-sourced citations are admissible only if (i) two independent origins agree on the cited content, and (ii) any proposal targeting $T_2$ or above is additionally flagged for human review at promotion. This closes the soundness hole where attacker-controlled web content passes verbatim-match verification while being untrustworthy.
\end{enumerate}
...
\label{sec:doctrine-compilation}
\textbf{Doctrine Compilation.} Promotion concludes by compiling the merged
doctrine into the active artifact set; compilation enforces all byte-cap
gates ($\mathbf{Cap}(T_i)$ per tier) and fails closed on violation. This
compilation step is what conformance test TC-15 exercises.
...
bodies remain auditably recoverable and never silently vanish. Pruned
rules that receive no hits across two consecutive windows are deleted;
all others are archived.
...
\begin{table}[H]
\centering
\caption{Additional Conformance Tests (TC-10 to TC-15)}
\begin{tabular}{@{}lll@{}}
\toprule
...
TC-17 & Curate staging isolation & Every $\mathbf{Persist}$ performed by \texttt{curate} lands in $\mathcal{A}'_{\text{artifact}} \setminus \mathcal{W}_{\text{workspace}}$; workspace byte-identical before/after. \\
